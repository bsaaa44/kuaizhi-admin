<template>
  <div>
    <div class='tools-bar'>
      ...
      <el-select class='status-selector' v-model='visableInp' placeholder='状态' @change='handleSearch' size='mini'>
    ...
    <el-dialog title='新增/修改分类' :visible.sync='showCategoryDialog' class='category-dialog' width='500'>
      <div class='inp-block'>
        ...
        <table border='0'>
          ...
          <tr>
            ...
            <td><el-input id='nameInp' placeholder="请输入排序大小（越小越靠前）" v-model='sortInp' class='inp'/></td>
          </tr>
        </table>
      </div>
      <div class='btn-block'>
  ...
  methods: {
    handlePut: function(scope,status){
      this.$confirm('是否执行该操作', '提示', {
          ...
          this.$utils.axiosRequest('POST', `/robot/category/visible`, '', data, res=>{
    ...
    handleEdit: function(){
      ...
      this.$utils.axiosRequest('POST', `/robot/category/store`, '', data, res=>{
         this.bindPageChange(this.currentPage)
    ...
    handleDelete: function(scope){
      this.$confirm('是否删除该分类', '提示', {
          ...
          this.$utils.axiosRequest('POST', `/robot/category/delete`, '', data, res=>{
            this.handleGetList()
          },res=>{
    ...
    handleGetList: function(data){
      ...
      this.$utils.axiosRequest('GET', `/robot/category/list`, data, '', res=>{
