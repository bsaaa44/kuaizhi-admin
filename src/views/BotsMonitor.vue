<template>
  <div>
    ...
    <el-table :data="list" v-loading="tableLoading" :height="maxHeight">
      ...
      <el-table-column prop="id" label="当前状态">
        <template slot-scope="scope">{{scope.row.status == 1?"运行中":"暂停中"}}</template>
      </el-table-column>
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="text" @click="handleDelete(scope)">删除</el-button>
          <el-button type="text" @click="handlePause(scope)" v-if="scope.row.status == 1">暂停</el-button>
          <el-button type="text" @click="handleStart(scope)" v-if="scope.row.status == 0">启用</el-button>
        </template>
      </el-table-column>
    </el-table>
