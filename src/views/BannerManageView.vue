<template>
  <div>
    <div class='tools-bar'>
      <el-button size='mini' @click='handleAdd()'>新增banner</el-button>
    </div>
      ...
      <el-table :data='dataList' v-loading='tableLoading' :height='maxHeight'>
        ...
        <el-table-column label='图片'>
          <template slot-scope='scope'>
            <img :src='scope.row.banner' class='table-img'/>
          </template>
        </el-table-column>
        <el-table-column label='包含机器人'>
          <template slot-scope='scope'>
            <el-button type='text' style="width:100%" @click='handleGetRobotList(scope)'>{{scope.row.bots_count}}</el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog title='banner机器人' :visible.sync='showRobotDialog' class='edit-dialog' width='500'>
    ...
    <el-dialog title='新增/修改机器人' :visible.sync='showBotEditDialog' class='edit-dialog' width='500'>
      <div class='config-block'>
        <table>
          <tr>
            <td>机器人ID</td>
            <td><el-input v-model='idInp' @input='handleGetBotDetail(idInp)'/></td>
          </tr>
          <tr>
            <td>机器人名称</td>
            <td>{{nameTips}}</td>
  ...
  methods: {
    ...
    handleBotDelete: function(scope){
      this.$confirm('是否执行此操作', '提示', {
        ...
        this.$utils.axiosRequest('POST', `/robot-recommend-banner/robot-delete`, '', data, res=>{
    ...
    handleBotEdit: function(scope){
      ...
      this.idInp = scope.row.bot_id
      this.sortInp = scope.row.sort
      this.handleGetBotDetail(scope.row.bot_id)
      this.showBotEditDialog = true
    },
    handleGetRobotList: function(scope,page){
      ...
      this.$utils.axiosRequest('GET', `/robot-recommend-banner/robot-list`, data, '', res=>{
    ...
    handlePutGoods: function(scope,type){
      this.$confirm('是否执行此操作', '提示', {
        ...
        this.$utils.axiosRequest('POST', `/robot-recommend-banner/visible`, '', data, res=>{
    ...
    handleGetList: function(data){
      this.$utils.axiosRequest('GET', `/robot-recommend-banners`, data, '', res=>{
    ...
    handleThemeConfirm: function(){
      let data = {
        banner_id: this.currentScope.row.id,
        bot_id: this.idInp,
        id: this.currentScope2==""?undefined:this.currentScope2.row.id,
        sort: this.sortInp
      }
      this.$utils.axiosRequest('POST', `/robot-recommend-banner/robot-store`, '', data, res=>{
        this.handleGetRobotList(this.currentScope,this.currentPage2)
    ...
    handleEdit: function(){
      let data = {
        banner: this.uploadCover,
        id: this.currentScope==""?undefined:this.currentScope.row.id,
        name: this.nameInp,
        sort: this.sortInp
      }
      this.$utils.axiosRequest('POST', `/robot-recommend-banner/store`, '', data, res=>{
        this.handleGetList({page: this.currentPage})
        this.showEditDialog = false
      },res=>{
